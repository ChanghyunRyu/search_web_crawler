...
class Frontier:
    def __init__(self):
        self.permit_site_en = ['CNN', 'CNBC', 'Daily Mail', 'The Guardian', 'ESPN', 'Reuters', 'CBS Sports',
    ...
    def getNewsLinkBySearch_En(self, keyword, queue, number):
        ...
        for i in range(len(self.permit_site_en)+1):
            ...
            print('first crawl thread start: ' + str(thread_start - start))
    ...
    def startCrawl(self, role, keyword, number=1):
        ...
        while True:
            if not self.url_queue.empty():
                if count == 0:
                    url_time = time.time()
                    print('queue detected: ' + str(url_time-start))
                    start = url_time
                count += 1
                t = threading.Thread(target=self.startConvertor, args=(self.url_queue.get(), ))
                t.start()
                threads.append(t)
            if count >= number:
                break
        ...
        print('content extract: ' + str(result_time-url_time))
        return self.results
